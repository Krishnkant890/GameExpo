...
interface AIService {
  ...
  generateImageAndScoreFromPrompt(
    prompt: string,
...
class GeminiService implements AIService {
  ...
  async generatePromptFromImage(
    ...
    const fallbackPrompt =
      "Create an image that matches the reference image composition, subject, lighting, and style.";
    ...
    try {
      ...
      for (const modelName of textModels) {
        try {
          ...
          console.error(
            `Gemini prompt model failed (${modelName}):`,
            modelError,
          );
        }
      }
  ...
  async generateImageAndScoreFromPrompt(
    prompt: string,
    referenceImageDataUrl: string,
  ): Promise<GenerateAndScoreResult> {
    const imageUrl = await this.generateImageFromPrompt(prompt);
    const scorePercentage = await this.compareImages(
    ...
    try {
      ...
      for (const modelName of scoreModels) {
        try {
          ...
          console.error(
            `Gemini score model failed (${modelName}):`,
            modelError,
          );
        }
      }
...
export const aiService: AIService = {
  ...
  generateImageAndScoreFromPrompt(
    ...
    return service.generateImageAndScoreFromPrompt(
      prompt,
      referenceImageDataUrl,
    );
  },
...
async function toInlineImagePart(
  imageUrl: string,
