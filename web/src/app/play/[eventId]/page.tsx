    ...
    if (status === 'registered') {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-background text-white p-12 text-center overflow-hidden">
                ...
                <motion.div variants={containerVariants} initial="initial" animate="animate" className="z-10 bg-glass p-10 rounded-[40px] border-2 border-primary/30 shadow-2xl">
                    ...
                    <h2 className="text-3xl font-black font-orbitron uppercase tracking-tighter mb-4 text-glow-blue">READY FOR BATTLE</h2>
                    <p className="text-white font-medium mb-6 uppercase tracking-widest text-sm italic">You are registered, {name}!</p>
                    <p className="text-primary/40 font-orbitron font-bold text-[10px] uppercase tracking-[0.3em] leading-relaxed">
                        Please proceed to the main screen <br />to start your turn.
                    </p>
                </motion.div>
            </div>
        );
            ...
            >
                ...
                <form onSubmit={handleJoin} className="space-y-6">
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full mt-10 bg-primary text-black py-6 rounded-2xl font-black font-orbitron text-lg uppercase tracking-widest hover:bg-white transition-all shadow-2xl relative group overflow-hidden"
                    >
                        <div className="absolute inset-x-0 h-1 bg-white/20 top-0 group-hover:animate-pulse" />
                        {loading ? <Loader2 className="animate-spin mx-auto" /> : 'REGISTER PLAYER'}
                    </button>
                </form>
            </motion.div>
        </div>
